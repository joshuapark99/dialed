...
This runbook hosts Dialed's public-beta hostname on one 64-bit Raspberry Pi behind a remotely managed Cloudflare Tunnel. The public hostname does not use Cloudflare Access; reserve Access for separate non-public endpoints. The Pi opens no inbound application, database, or SSH port. GitHub Actions publishes images; a systemd timer on the Pi pulls and verifies them.
...
## 3. Configure the public Cloudflare Tunnel
...
Leave the route's **Protect with Access** option disabled. The public beta uses the named tunnel for TLS and private origin connectivity while allowing anyone to load the application. Dialed's own Google login protects cloud accounts and account-scoped data; anonymous data remains local to the visitor's browser. Application-level rate limits and sync quotas provide the initial public-beta abuse guardrails.

### Access boundaries

Do not attach the public application hostname to an Access application. Continue to use Access on separate non-public staging, administrative, or operational hostnames if those are added later.
...
Enable the Cloudflare public-hostname route with **Protect with Access** disabled. In a private browser window, confirm the app loads without an edge login and Google login returns to Dialed. Verify that anonymous data remains local to that browser and that authenticated account data is available only after Dialed login.

The CI deployment check polls these endpoints without authentication headers:

- `/healthz` for web status `ok`;
- `/api/readyz` for API/database status `ready`.

Both responses must report the Git commit that produced the images. A published image is not considered deployed until this public external check succeeds.
...
Remove the Cloudflare hostname and tunnel after the connector is offline. Revoke Google credentials if they are dedicated to the POC.
